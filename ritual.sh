...
EOF
  ...
  if docker ps -a | grep -q 'deploy-redis-1' && docker ps -a | grep -q 'deploy-fluentbit-1'; then
    ...
    echo -e "${err}\нКонтейнеры запущены неправильно. Продолжение...${end}" | tee -a "$log_file"
  fi
  ...
  echo -е "${fmt}\нУстановка Foundry${end}" | tee -а "$log_file"
  cd /root/
...
view_status() {
  echo -е "${fmt}Проверка статуса ноды...${end}"
  curl localhost:4000/health
}

# Меню
PS3='Пожалуйста, выберите опцию: '
options=("Ввести приватный ключ" "Установить ноду Ritual" "Просмотреть логи ноды Ritual" "Просмотреть статус ноды Ritual" "Выйти из установочного скрипта")
...
do
  case $opt in
    ...
    "Просмотреть логи ноды Ritual")
      view_logs
      ;;
    "Просмотреть статус ноды Ritual")
      view_status
      ;;
